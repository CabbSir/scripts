...
config = {
    "exe_path": "D:\\Program Files\\3DF Zephyr Aerial\\3DF Zephyr Aerial.exe",  # 3df的exe文件路径
    "data_path": "I:\\245次整理好的数据\\第四次（处理好的）\\第五次\\3.18农科院小麦\\双拍2\\上\\102CANON\\",  # 数据存放路径
    "use_mask": True,  # 是否使用蒙版
    "dense_time": 70 * 60,  # 重建最大时长 默认70分钟 根据情况调节
}

# 常量定义
FREE_CPU = 4  # 空闲情况下的cpu利用率
...
def export_points(win, path):
    ...
    s()
    send_keys(path + "\\" + str(int(time.time())) + ".ply")
    s()
    send_keys("{ENTER}")
    s(5)
...
def build_dense(a, w, path):
    ...
    try:
        ...
        # 判断是否有ply文件
        build_ply = False
        for y in os.listdir(config['data_path'] + folder):
            if '.ply' in y:
                build_ply = True
                break
        return build_ply
...
if __name__ == '__main__':
    ...
    for folder in os.listdir(config['data_path']):
        if len(folder.split('.')) == 2:
            # 非文件夹 跳过
            continue
        if len(os.listdir(config['data_path'] + folder)) <= 10:
            continue
